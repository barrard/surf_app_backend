...
async function fetchStation (stationId) {
	...
	function getRowDataFromTable (table) {
		...
		Array.from(table).forEach((child) => {
			...
			const isTable = child.name === 'table'
			if (isTable) {
				tableCount++
			}
	...
	dataArray.reverse().forEach((data) => {
		// get the GMT time
		// console.log(data);
		const { Hour, Min, DD, MM, year } = data
		...
		Array.from(children).forEach((child) => {
			...
			if (child.name === 'caption') {
				...
				if (metaData.startsWith('<a href')) {
					...
					const [_, __, dateTime] = metaData.split('<br>')
					const [_time, _onDate] = dateTime.split('GMT')
					time = _time.trim()

					onDate = _onDate
				} else {
					const [_, caption] = metaData.split('<br>')
					// console.log(caption);
					const [_time, _onDate] = caption.split('GMT')
					time = _time.trim()
					onDate = _onDate
				}
				...
				const [_month, _day, _year] = date.split('/')
				month = _month.trim()
				day = _day.trim()
				year = _year.trim()
...
async function fetchStationWithNotifications (stationId) {
	...
	try {
		...
		if (result.notified > 0) {
			log(
				`Sent ${result.notified} notifications for station ${stationId} (period: ${result.period}s)`
			)
		}
	} catch (error) {
