...
async function findBuoysNear (args = {}) {
	...
	const buoys = await BuoyModel.find(
		{
			// coords: {
			//     $near: {
		...
		try {
			...
			if (recentlyFetched[stationId]) {
				...
				const stationData =
                    ...
                    await waveDataService.fetchStationWithNotifications(
                    	stationId
                    )
				...
				const mergedMostRecent = sortedData.reduce((merged, current) => {
...
async function cleanData (data) {
	...
	const cleanData = {}
	for (const id in station_id_obj) {
